...
>
    ...
    <template x-if="screen === 'catalog'">
        <div class="flex flex-col gap-3">
            <template x-if="catalog.length === 0">
                <p class="rounded-xl bg-tg-section p-6 text-center text-tg-hint" x-text="t('empty')"></p>
            </template>
            ...
            <template x-for="service in visibleServices" :key="service.id">
                <article class="flex flex-col gap-3 rounded-xl bg-tg-section p-4">
                    <div class="flex flex-col gap-1">
                        <h2 class="font-semibold" x-text="tr(service.name)"></h2>
                        <p class="text-sm text-tg-hint" x-show="tr(service.description)" x-text="tr(service.description)"></p>
                    </div>

                    <div class="flex items-center justify-between gap-3">
                        <div class="flex flex-col">
                            <span class="font-semibold" x-text="money(service.price)"></span>
                            <span class="text-xs text-tg-hint" x-show="service.duration" x-text="t('minutes', { count: service.duration })"></span>
                        </div>
                        ...
                        <button
                            ...
                            x-text="t('add_to_cart')"
                        ></button>

                        <div x-show="quantity(service.id) > 0" class="flex items-center gap-3">
                            <button type="button" @click="remove(service.id)" :aria-label="t('remove_one')"
                            ...
                            <button type="button" @click="add(service.id)" :aria-label="t('add_one')"
    ...
    <template x-if="screen === 'cart'">
        <div class="flex flex-col gap-3">
            ...
            <form @submit.prevent="submit()" class="flex flex-col gap-3 rounded-xl bg-tg-section p-4">
                <h2 class="text-lg font-semibold" x-text="t('contacts')"></h2>

                <label class="flex flex-col gap-1">
                    <span class="text-sm text-tg-hint" x-text="t('name')"></span>
                ...
                <label class="flex flex-col gap-1">
                    <span class="text-sm text-tg-hint" x-text="t('phone')"></span>
                ...
                <label class="flex flex-col gap-1">
                    <span class="text-sm text-tg-hint" x-text="t('comment')"></span>
                    <textarea x-model="form.comment" rows="3" maxlength="1000" :placeholder="t('comment_placeholder')"
    ...
    <template x-if="screen === 'done'">
        <section class="flex flex-col items-center gap-3 rounded-xl bg-tg-section p-6 text-center">
            ...
            <h2 class="text-xl font-semibold" x-text="t('done_title', { number: order.number })"></h2>
            <p class="text-tg-hint" x-show="!paid" x-text="t('done_text', { total: order.total })"></p>
            <p class="text-tg-hint" x-show="paid" x-text="t('paid_text')"></p>
        </section>
    </template>
    ...
    <button
        ...
        x-text="t('cart_button', { count: cartCount, total: money(cartTotal) })"
    ></button>
</div>
</body>
</html>
